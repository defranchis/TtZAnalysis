...
##here do qsub or dirty &
	   ...
	   # array=( "${array[@]}" "jack" )
		outnames=( "${outnames[@]}" "${outname}" );
                cp $templatesDir/job_HTCondor_BRIL.sh jobscripts
		sed -e "s;##WORKDIR##;${workdir};" < $templatesDir/job_HTCondor_BRIL.sh > jobscripts/job_HTCondor_BRIL.sh
		chmod u+x jobscripts/job_HTCondor_BRIL.sh
		sed -e "s;##WORKDIR##;${workdir};" -e "s/#CHANNEL#/${channel}/" -e "s/#ENERGY#/${energy}/" -e "s/#TOPMASS#/${topmass}/" -e "s/#SYST#/${syst}/" -e "s/#ADDPARAMS#/${addParameters}/" -e "s/#BATCHNAME#/${outname}/" < $templatesDir/HTCondor_BRIL.submit > jobscripts/${outname}
