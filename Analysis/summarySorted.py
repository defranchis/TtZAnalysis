...
if not mass : f1 = open('xsecFit_tab13TeV.tex','r')
...
for line in l1_short:
    ...
    pull_all = np.append(pull_all,float(pull))
    constrain_all = np.append(constrain_all,float(constrain))
    contribution_all = np.append(contribution_all,abs(float(contribution)))

    idx = np.argsort(contribution_all)
    idx = idx[::-1]

    name_all = np.array(name_all)[idx]
    pull_all = np.array(pull_all)[idx]
    constrain_all = np.array(constrain_all)[idx]
    contribution_all = np.array(contribution_all)[idx]

    
histo_all = ROOT.TH1F('histo_all','histo_all',maxSyst,-.5,-.5+maxSyst)
...
if not os.path.exists('systPlots'):
    os.makedirs('systPlots')
...
histo_all.SetTitle('fit summary - biggest')
histo_all.SetMaximum(3.)
histo_all.SetMinimum(-3.)
...
if mass:
    c1.SaveAs('systPlots/fitSummary_xsec.pdf','pdf')
    c1.SaveAs('systPlots/fitSummary_xsec.png','png')
else:
    c1.SaveAs('systPlots/fitSummary_mass.pdf','pdf')
    c1.SaveAs('systPlots/fitSummary_mass.png','png')
